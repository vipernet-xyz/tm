...
LD_FLAGS = -X github.com/vipernet-xyz/tm/version.TMCoreSemVer=$(VERSION)
BUILD_FLAGS = -mod=readonly -ldflags "$(LD_FLAGS)"
HTTPS_GIT := https://github.com/vipernet-xyz/tm.git
CGO_ENABLED ?= 0
...
draw_deps:
	...
	@goviz -i github.com/vipernet-xyz/tm/cmd/tendermint -d 3 | dot -Tpng -o dependency-graph.png
.PHONY: draw_deps
...
format:
	...
	find . -name '*.go' -type f -not -path "*.git*"  -not -name '*.pb.go' -not -name '*pb_test.go' | xargs goimports -w -local github.com/vipernet-xyz/tm
.PHONY: format
